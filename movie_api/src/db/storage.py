...
class FilmSearchStorage(ElasticSearchStorage):
    async def get_by_id(
        self, instance_id: str, filter_suspicious: bool = True, **kwargs
    ) -> Optional[dict]:
        film = await super().get_by_id(instance_id)
        if filter_suspicious and film and film["suspicious"]:
            return None
        return film

    async def bulk_get_by_ids(
        self, ids: List[str], filter_suspicious: bool = True, **kwargs
    ) -> List[dict]:
        films = await super().bulk_get_by_ids(ids)
        if filter_suspicious:
            for film in films:
                if not hasattr(film, "suspicious"):
                    logger.debug(f"film with no susp: {film}")
            return [film for film in films if not film["suspicious"]]
        else:
            return films
